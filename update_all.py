...
"""
    ...
    for _, row in df.iterrows():
        ...
        pattern = re.escape(display) + r""" — <span data-lang="cn">累积收益 [\d.]+%</span><span data-lang="en">Cumulative Return [\d.]+%</span>"""
...
def main():
    ...
    # 4. 推送两个仓库
    print("\n[4/4] 推送仓库...")
    commit_msg = f"Update strategy results {today}"
    git_commit_push(TRADING_DIR, commit_msg)
    git_commit_push(WEBSITE_DIR, commit_msg)
